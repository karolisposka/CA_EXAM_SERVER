...
const {
  loginSchema,
  registerSchema,
} = require("../../Middleware/Validation/ValidationSchemas/userValidation");
...
router.post("/login", validation(loginSchema), async (req, res) => {
  try {
    ...
    const [data] = await con.execute(
      `SELECT id, password FROM users WHERE email=${mysql.escape(
        req.body.email
      )} LIMIT 1`
    );
    await con.end();
...
router.post("/register", validation(registerSchema), async (req, res) => {
  try {
    ...
    const [data] =
      await con.execute(`INSERT INTO users (email, password, username)
    ...
    return res.send({ msg: "registration successfully completed" });
  } catch (err) {
    console.log(err);
    return res
      .status(500)
